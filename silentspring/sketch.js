...
function drawRadial(barValue, barDate) {
    ...
    if (frameCount <= 200) {
        maxValue = constrain(frameCount * 2, 0, 500);
    } else {
        maxValue = 400;
    }
    var offset = 120;

    var dataMultiplier = (windowHeight / 0.5 - offset - padding) * 6;
    ...
    for (var i = 0; i < barValue.length; i = i + 1) {
        ...
        var finalHeight = currentData / (dataMultiplier * 800);
